...
class sarsaAgent():
    ...
    def __init__(self):
        ...
        self.epsilon_T1 = 1e-4
        self.epsilon_T2 = 1e-6
        self.learning_rate_T1 = 0.1
        self.learning_rate_T2 = 0.02
        temp=int(19*15)
        self.weights_T1 = np.zeros((3,temp))
        temp=int(temp*4)
        self.weights_T2 = np.zeros((3,temp))
        self.discount = 1.0
    ...
    def get_better_features(self, obs):
        s=np.zeros((4,19,15))
        for i,a1 in enumerate([1.2,1.25]):
            for j,a2 in enumerate([0.07,0.075]):
                t1=(obs[0]+a1)*10
                t2=(obs[1]+a2)*100
                s[i*2+j,int(t1),int(t2)]=1
        #print(s.shape)
        return s.flatten()
    ...
    def choose_action(self, state, weights, epsilon):
        ...
        if(p<=epsilon):
            ...
            #print(weights.shape)
            t=[np.dot(i,state) for i in weights]
            s=argmax(t)
        #print(s)
        return s
    ...
    def sarsa_update(self, state, action, reward, new_state, new_action, learning_rate, weights):
        weights[action]=weights[action]+learning_rate*((reward+np.dot(weights[new_action],new_state)-np.dot(weights[action],state))*state)
        return weights
